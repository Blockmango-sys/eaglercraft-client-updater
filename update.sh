#!/bin/bash

echo "Please install Git before running this file, and ONLY run this on a Git repository."

# Create a new branch
git checkout -b vanilla-eagler-src
if [ $? -ne 0 ]; then
    echo "echo Please install Git before running this file. Exiting..."
    exit 1
fi

# Download the ZIP file
echo "Downloading Eaglercraft workspace..."
curl -L -o master.zip https://github.com/Eaglercraft-Archive/EaglercraftX-1.8-workspace/archive/refs/heads/master.zip

# Check if the download was successful
if [ $? -ne 0 ]; then
    echo "Error downloading the file. Exiting..."
    exit 1
fi

# Extract the ZIP file
echo "Extracting the ZIP file..."
unzip master.zip -d .

# Check if the extraction was successful
if [ $? -ne 0 ]; then
    echo "Error extracting the file. Exiting..."
    exit 1
fi

# Clean up the ZIP file
echo "Cleaning up..."
rm master.zip

# Prompt user for the main branch name
read -p "Enter the name of your main branch (e.g., main or master): " default_branch

# Switch back to the default branch
git checkout $default_branch

# Merge the newly created branch
git merge vanilla-eagler-src --allow-unrelated-histories

# Check if the merge was successful
if [ $? -ne 0 ]; then
    echo "Error during merge. Exiting..."
    exit 1
fi

echo "Done. Use GitHub Desktop and VSCode to fix conflicts."
